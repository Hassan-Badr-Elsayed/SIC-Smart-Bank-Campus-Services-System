...
To close the system, enter exit""")
    ...
    if option == "register":
        ...
        if not login_success:
            ...
            print(f"\nHello {name}! Welcome back (Logged in as {role}).")

            # Admin Routing
            if role == "Admin":
                reports_menu()

            # User & VIP Operations Routing
            elif role in ["User", "VIP"]:
                while True:
                    print(f"\n*************** Welcome back {name} ({role}) ***************")
                    print("[0] Deposit\n[1] Withdraw\n[2] Transfer\n[3] Transaction history\n[4] Branch/ATM status\n[5] Update personal info\n[6] Exit")

                    user_input = input("Please enter choice: ").strip()

                    if user_input == Deposit:
                        raw_dep = input("Please enter amount and currency (e.g. 100 USD): ").split()
                        if len(raw_dep) != 2 or not raw_dep[0].isdigit():
                        ...
                        transfer_val = input("Enter Transfer value: ").strip()
                        if not transfer_val.isdigit():
                            print("Invalid Transfer Value.")
                            continue
                            
                        transfer_value = int(transfer_val)
                        if transfer_value <= 0:
                            print("Invalid value.")
                        elif transfer_value > current_user["wallet"]["balance"]:
                            print("You don't have enough money.")
                        else:
                            current_user["wallet"]["balance"] -= transfer_value
                            target_user["wallet"]["balance"] += transfer_value
                            
                            current_user["history"].append({
                                "type": "transfer",
                                "details": f"Transfer to {target_user['profile']['name']}: {transfer_value} EGP"
                            })
                            target_user["history"].append({
                                "type": "transfer",
                                "details": f"Transfer from {current_user['profile']['name']}: {transfer_value} EGP"
                            })
                            print("Transfer success.")

                    elif user_input == Transaction_history:
                        if not current_user["history"]:
                            print("You don't have Transaction history.")
                        else:
